...
Schema Registry и Kafka Connect (Debezium) входят в корневой [`docker-compose.yml`](../docker-compose.yml). Достаточно из каталога репозитория:

```bash
docker compose up -d
```

Файлы [`infra/cdc/docker-compose.cdc.yml`](../infra/cdc/docker-compose.cdc.yml) и [`infra/metadata/atlas/docker-compose.atlas.yml`](../infra/metadata/atlas/docker-compose.atlas.yml) оставлены пустыми (legacy): не добавляйте их повторно в `-f`, иначе получите дубли сервисов.
